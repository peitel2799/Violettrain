...
export default function AdminPricingPage() {
  ...
  const handleAdd = async () => {
    if (!addValues.routeId || !addValues.cabinClassId) {
      setAddError('Please select both a route and a cabin class.')
      return
    }
    setSaving(true)
    setAddError('')
    ...
    try {
      ...
      if (res.ok || res.status === 201) {
        ...
        setMessage('Price route added successfully!')
        setTimeout(() => setMessage(''), 3000)
      } else {
        setAddError(data.error || 'Failed to add price route.')
      }
    } catch {
      setAddError('Failed to add price route.')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="p-6 lg:p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900" style={{ fontFamily: 'var(--font-serif)' }}>Pricing</h1>
          <p className="text-sm text-gray-500 mt-0.5">Manage route and cabin pricing tiers.</p>
        </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-left text-xs text-gray-500 uppercase bg-gray-50">
                    <th className="px-5 py-3 font-medium">Cabin Class</th>
                    <th className="px-5 py-3 font-medium">Base Price</th>
                    <th className="px-5 py-3 font-medium">Peak Price</th>
                    <th className="px-5 py-3 font-medium">Season Factor</th>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden">
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">Cabin Class</label>
                <select
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1.5">Base Price (VND)</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1.5">Peak Price (VND)</label>
                  <input
                    type="number"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">Season Factor</label>
                <input
                  type="number"
                  step="0.1"
                  min="0.5"
                  ...
                  className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500"
                />
                <p className="text-xs text-gray-400 mt-1">e.g. 1.0 = normal, 1.5 = 50% peak season</p>
              </div>
            </div>
