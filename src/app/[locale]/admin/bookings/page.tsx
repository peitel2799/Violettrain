        ...
        <div className="p-6 space-y-6">
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-violet-50 rounded-xl p-4">
              <p className="text-xs text-violet-500 font-medium mb-1">{t('route')}</p>
              <p className="font-semibold text-gray-900">{booking.routeName}</p>
              <p className="text-sm text-gray-500 mt-1">
                {booking.trainNumber} &middot; {booking.departureDate} at {booking.departureTime}
              </p>
            </div>
            <div className="bg-gold-50 rounded-xl p-4">
              <p className="text-xs text-gold-600 font-medium mb-1">{t('totalAmount')}</p>
              <p className="text-2xl font-bold text-gray-900">{formatCurrency(booking.pricing.total)}</p>
              <p className="text-xs text-gray-500 mt-1">
                Sub: {formatCurrency(booking.pricing.subtotal)} &middot; Disc: -{formatCurrency(booking.pricing.discount)} &middot; Tax: {formatCurrency(booking.pricing.tax)}
              </p>
            </div>
          </div>
          ...
          <div>
            <h4 className="text-sm font-semibold text-gray-700 mb-3">
              {t('passengersTitle', { count: booking.passengers.length })}
            </h4>
            <div className="space-y-2">
              {booking.passengers.map((p, i) => (
                <div key={i} className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
                  ...
                  <span className="text-xs bg-gray-200 text-gray-600 px-2 py-0.5 rounded-full">{p.type}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            <h4 className="text-sm font-semibold text-gray-700 mb-3">{t('paymentTitle')}</h4>
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <div className="p-3 bg-gray-50 rounded-xl">
                <p className="text-xs text-gray-400">{t('transactionId')}</p>
                <p className="font-mono text-xs text-gray-700">{booking.payment.transactionId || t('na')}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-gray-400 text-sm">{t('noBookings')}</p>
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-xs text-gray-500 uppercase bg-gray-50">
                  ...
                  <th className="px-5 py-3 font-medium">{t('passengers')}</th>
                  <th className="px-5 py-3 font-medium">{t('amount')}</th>
                  <th className="px-5 py-3 font-medium">{t('bookingStatus')}</th>
                  <th className="px-5 py-3 font-medium">{t('paymentStatus')}</th>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between px-5 py-4 border-t border-gray-100">
            <p className="text-xs text-gray-500">
              {t('showing', { from: (page - 1) * 15 + 1, to: Math.min(page * 15, total), total })}
            </p>
            <div className="flex items-center gap-2">
              <button
